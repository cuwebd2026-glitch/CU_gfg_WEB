...
export default function EventCard({ event, index }: EventCardProps) {
  return (
    <motion.article
      ...
      className="surface-card overflow-hidden group hover:border-[var(--gfg-green)]/50 w-full"
    >
      <div className="relative h-64 md:h-80 overflow-hidden bg-muted/10">
        ...
        />
        <span
          className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-semibold z-20 ${
      ...
      <div className="p-4">
        <span className="text-xs font-mono text-[var(--gfg-green)] uppercase tracking-wider">{event.category}</span>
        <h3 className="font-display font-bold text-lg text-foreground mt-2 mb-2">{event.title}</h3>
        ...
        <div className="mt-4 pt-4 border-t border-border/50 space-y-2 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span><span className="font-semibold text-foreground">Date:</span> {event.date}</span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span><span className="font-semibold text-foreground">Venue:</span> {event.location}</span>
          </div>
        </div>
      </div>
    </motion.article>
